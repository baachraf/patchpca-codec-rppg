"""
ubfc_parser.py — UBFC-rPPG Parser (thin dataset handler)
============================================================
All CV logic lives in shared_scripts/face_vision_degraded.py.

Degradation variants (applied to all subjects when USE_DEGRADATION=True):
  'none'        — no transform (original clean uncompressed RGB)
  ...
  'mpeg4_low'   — MPEG-4 encode/decode at ~85 kbps
  'mpeg4_med'   — MPEG-4 encode/decode at ~200 kbps
  'frame_drop'  — randomly drop DROP_RATE% of frames
  'flicker'     — sinusoidal luminance flicker at FLICKER_HZ
  'gauss_noise' — additive Gaussian read noise

Note: UBFC-rPPG is stored as raw uncompressed RGB (no codec) —
the cleanest possible signal. Degradation here directly tests
whether codec artifacts flip PCA's advantage on clean data.
"""
...
from pipeline_config import (
    UBFC_RPPG_DATASET_ROOT  as DATASET_ROOT,
    UBFC_RPPG_OUT_DIR       as OUT_DIR,
    N_SUBJECTS_LIMIT,
...
try:
    from pipeline_config import MICRO_PILOT_SIDS_UBFC_RPPG
except ImportError:
    MICRO_PILOT_SIDS_UBFC_RPPG = None
NUM_WORKERS      = WORKERS['ubfc_rppg']
INCLUDE_SUBJECTS = sorted(MICRO_PILOT_SIDS_UBFC_RPPG) if MICRO_PILOT_SIDS_UBFC_RPPG else []
...
CFG = FaceVisionConfig(
    ...
    frame_transform   = None,        # set at runtime
)

# ==============================================================================
# FRAME TRANSFORMS — identical helpers as mcd_parsing.py
# ==============================================================================
...
def _build_transform(variant, fps_nom):
    ...
    # ── NEW variants [CH-2, CH-3] ─────────────────────────────────────────────
        ...
        # [CH-2] Pure 4:2:0 chroma subsampling only — no DCT, no quantisation.
        # Isolates the chrominance-ratio corruption hypothesis.
        return _transform_yuv420_chroma
    elif variant == 'h265_intra':
        # [CH-3a] H.265 all-intra (GOP=1, CRF=28). Spatial DCT blocking only.
        # No inter-frame prediction noise. Baseline for spatial-only artifacts.
        return _make_h265_transform(gop_size=1, crf=28)
    elif variant == 'h265_gop15':
        # [CH-3b] H.265 GOP=15 (CRF=28). Full inter-frame prediction active.
        # Introduces P-frame motion residuals + temporal noise.
        return _make_h265_transform(gop_size=15, crf=28)
    elif variant == 'h265_gop15_hq':
        # [CH-3c] H.265 GOP=15 lower CRF=18 (higher quality).
        # Tests whether inter-frame noise alone (without heavy quantisation)
        # is sufficient to trigger PatchPCA's advantage.
...
def load_gt_ubfc(gt_path, n_frames, fps_nom):
    ...
    hr_raw  = (np.array([float(x) for x in lines[1].strip().split()], dtype=np.float64)
               if len(lines) > 1 and lines[1].strip()
               else np.full(len(ppg_raw), np.nan))
    ts_raw  = (np.array([float(x) for x in lines[2].strip().split()], dtype=np.float64)
               if len(lines) > 2 and lines[2].strip() else None)

    if ts_raw is not None and len(ts_raw) > 1:
        if len(ts_raw) == n_frames:
            frame_times = ts_raw.copy()
        else:
            gt_idx      = np.linspace(0, n_frames - 1, len(ts_raw))
            frame_times = interp1d(gt_idx, ts_raw, kind='linear',
                                   bounds_error=False,
                                   fill_value=(ts_raw[0], ts_raw[-1]))(np.arange(n_frames))
    else:
        frame_times = np.arange(n_frames, dtype=np.float64) / fps_nom

    if ts_raw is not None and len(ts_raw) > 1:
        gt_ppg = interp1d(ts_raw, ppg_raw, kind='linear', bounds_error=False,
                          fill_value=(ppg_raw[0], ppg_raw[-1]))(frame_times).astype(np.float32)
        gt_bpm = interp1d(ts_raw, hr_raw,  kind='linear', bounds_error=False,
                          fill_value=(hr_raw[0],  hr_raw[-1]))(frame_times).astype(np.float32)
    else:
        t_src  = np.linspace(0, 1, len(ppg_raw))
        t_dst  = np.linspace(0, 1, n_frames)
        gt_ppg = interp1d(t_src, ppg_raw, kind='linear', bounds_error=False,
                          fill_value='extrapolate')(t_dst).astype(np.float32)
        gt_bpm = interp1d(t_src, hr_raw,  kind='linear', bounds_error=False,
                          fill_value='extrapolate')(t_dst).astype(np.float32)
...
def _parse_subject(args):
    sid, subject_folder, out_dir = args
    out_path   = os.path.join(out_dir, f'subject{sid}.csv')

    if os.path.exists(out_path):
        return (sid, 'SKIP', 0)

    video_path = os.path.join(subject_folder, VIDEO_FILENAME)
    gt_path    = os.path.join(subject_folder, GT_FILENAME)

    if not os.path.exists(video_path):
        return (sid, 'FAIL: no video', 0)
    if not os.path.exists(gt_path):
        return (sid, 'FAIL: no GT', 0)
    ...
    try:
        ...
        if fps_nom <= 0 or n_frames == 0:
            return (sid, 'FAIL: bad video meta', 0)
        ...
        gt_ppg, gt_bpm, frame_times = load_gt_ubfc(gt_path, n_frames, fps_nom)
        ...
        if 'SUCCESS' in result:
            try:
                n_rows = int(result.split('|')[1].strip().split()[0])
            except Exception:
                pass
        return (sid, 'OK' if 'SUCCESS' in result else result, n_rows)
...
# ==============================================================================
# MAIN — loops over all variants from degradation_config.json
# ==============================================================================
...
def main():
    ...
    for name in sorted(os.listdir(DATASET_ROOT)):
        ...
        if os.path.isdir(full) and name.startswith('subject'):
            try:
                all_sids.append(int(name.replace('subject', '')))
            except ValueError:
                pass
    ...
    for fname in (os.listdir(out_dir) if os.path.exists(out_dir) else []):
        if fname.endswith('.csv'):
            try:
                done.add(int(fname.replace('subject','').replace('.csv','')))
            except Exception:
                pass

    pending = [s for s in all_sids if s not in done]
    log(f'UBFC-rPPG — {len(pending)}/{len(all_sids)} pending | {len(VARIANTS_TO_RUN)+1} variants each')

    if not pending:
        log('All done.')
    else:
        n_ok = 0; n_fail = 0; n_skip = 0
        work = [(sid, os.path.join(DATASET_ROOT, f'subject{sid}'), out_dir)
                for sid in pending]
        with Pool(processes=min(NUM_WORKERS, len(pending))) as pool:
            bar = tqdm(pool.imap_unordered(_parse_subject, work),
                       total=len(work), desc='UBFC-rPPG', unit='subj',
                       dynamic_ncols=True)
            for sid, status, n_rows in bar:
                if 'FAIL' in status:
                    n_fail += 1
                    log(f'  subject{sid:02d}  {status}')
                elif status == 'SKIP':
                    n_skip += 1
                else:
                    n_ok += 1
                bar.set_postfix(ok=n_ok, skip=n_skip, fail=n_fail,
                                last=f's{sid}', rows=n_rows)
        log(f'Parse complete. ok={n_ok} skip={n_skip} fail={n_fail}')
